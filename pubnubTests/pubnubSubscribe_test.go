// Package pubnubTests has the unit tests of package pubnubMessaging.
...
func TestSubscriptionConnectStatus(t *testing.T) {
	pubnubInstance := pubnubMessaging.NewPubnub("demo", "demo", "", "", false, "")
...
func TestSubscriptionForSimpleMessageWithCipher(t *testing.T) {
	pubnubInstance := pubnubMessaging.NewPubnub("demo", "demo", "", "enigma", false, "")
...
func PublishComplexMessage(pubnubInstance *pubnubMessaging.Pubnub, t *testing.T, channel string, testName string, cipherKey string, responseChannel chan string) {
...
func SendMultipleResponse(t *testing.T, encrypted bool) {
	...
	pubnubInstance := pubnubMessaging.NewPubnub("demo", "demo", "", cipher, false, "")
	...
	for {
		...
		case value, ok := <-returnChannel:
			...
			if string(value) != "[]" {
				...
				if err == nil {
					retVal = pubnubMessaging.ParseInterfaceData(s[0])
					return retVal, nil
				}
				retError = err
				return "", retError
		...
		case value, ok := <-errorChannel:
			...
			if string(value) != "[]" {
				retError = fmt.Errorf(_timeoutMessage)
				return "", retError
			}
			break
		}
...
//
// parameters:
// pubnubInstance: pubnubMessaging.Pubnub instance.
...
func ParseSubscribeForTimetoken(pubnubInstance *pubnubMessaging.Pubnub, pubnubChannel string, returnChannel chan []byte, errorChannel chan []byte, testName string, responseChannel chan string) {
	for {
		select {
		case value, ok := <-returnChannel:
			if !ok {
		...
		case value, ok := <-errorChannel:
			...
			if string(value) != "[]" {
				newPubnubTest := &pubnubMessaging.PubnubUnitTest{}
...
func SendMultiplexingRequest(t *testing.T, testName string, ssl bool, encrypted bool) {
	...
	pubnubInstance := pubnubMessaging.NewPubnub("demo", "demo", "", cipher, ssl, "")
	returnSubscribeChannel := make(chan []byte)
	errorChannelSub := make(chan []byte)
	responseChannelSub := make(chan string)
	waitChannelSub := make(chan string)
