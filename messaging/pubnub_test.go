...
func TestCreateSubscribeURLReset(t *testing.T) {
	...
	assert.Equal("/v2/subscribe/demo/ch/0?channel-group=cg&uuid=testuuid&tt=0&tr=4&filter-expr=aoi_x%20%3E%3D%200&heartbeat=10&state=%7B%22ch%22%3A%7B%22k%22%3A%22v%22%7D%7D&pnsdk=PubNub-Go%2F3.12.0", b)
...
func TestCreateSubscribeURL(t *testing.T) {
	...
	//log.SetOutput(os.Stdout)
	//log.Printf("b:%s, tt:%s", b, tt)
	assert.Equal("/v2/subscribe/demo/ch/0?channel-group=cg&uuid=testuuid&tt=14767805072942467&tr=4&filter-expr=aoi_x%20%3E%3D%200&pnsdk=PubNub-Go%2F3.12.0", b)
	assert.Equal(senttt, tt)
}
...
func TestCreateSubscribeURLFilterExp(t *testing.T) {
	...
	//log.SetOutput(os.Stdout)
	//log.Printf("b:%s, tt:%s", b, tt)
	assert.Equal("/v2/subscribe/demo/ch/0?channel-group=cg&uuid=testuuid&tt=14767805072942467&tr=4&filter-expr=aoi_x%20%3E%3D%200%20AND%20aoi_x%20%3C%3D%202%20AND%20aoi_y%20%3E%3D%200%20AND%20aoi_y%3C%3D%202&pnsdk=PubNub-Go%2F3.12.0", b)
	assert.Equal(senttt, tt)
}
