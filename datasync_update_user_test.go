...
func TestUpdateUserBuildPathQueryBody(t *testing.T) {
	assert := assert.New(t)
...
func TestUpdateUserHTTPMethodAndOperation(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())
	o := newUpdateUserBuilder(pn)
...
func TestUpdateUserExecuteValidationError(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())
