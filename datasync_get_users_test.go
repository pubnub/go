...
func TestGetUsersFilterQueryParam(t *testing.T) {
	...
	o := newGetUsersBuilder(pn).Filter("status == 'active' && location == 'Pune'")
	query, err := o.opts.buildQuery()
	assert.Nil(err)
...
func TestGetUsersResponseParsing(t *testing.T) {
	...
	jsonBytes := []byte(`{"data":[{"id":"alice","status":"active","entityClass":"User","entityClassVersion":1,"entityClassLevel":"SubKey","eTag":"1","payload":{"name":"Alice"}}],"links":{"self":"/self","next":"/next"},"meta":{"has_next":true,"next_cursor":"TjIw","limit":20}}`)
