...
func TestSetMembershipBuildPathQueryBody(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())

	o := newSetMembershipBuilder(pn).
		ID("m-123").
...
func TestSetMembershipHeaders(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())

	o := newSetMembershipBuilder(pn).ID("m-123").RelationshipClassVersion(1).IfMatchETag("1")
...
func TestSetMembershipHTTPMethodAndOperation(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())
	o := newSetMembershipBuilder(pn)
	assert.Equal("PUT", o.opts.httpMethod())
	assert.Equal(PNSetDataSyncMembershipOperation, o.opts.operationType())
}

func TestSetMembershipValidate(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())

	o := newSetMembershipBuilder(pn).RelationshipClassVersion(1)
	assert.Contains(o.opts.validate().Error(), StrMissingMembershipID)

	o2 := newSetMembershipBuilder(pn).ID("m-123").RelationshipClassVersion(0)
	assert.Contains(o2.opts.validate().Error(), StrInvalidRelationshipClassVersion)

	pn.Config.SubscribeKey = ""
	o3 := newSetMembershipBuilder(pn).ID("m-123").RelationshipClassVersion(1)
	assert.Contains(o3.opts.validate().Error(), StrMissingSubKey)
}

func TestSetMembershipExecuteValidationError(t *testing.T) {
	assert := assert.New(t)
	pn := NewPubNub(NewDemoConfig())

	_, _, err := pn.DataSync.SetMembership().RelationshipClassVersion(1).Execute()
