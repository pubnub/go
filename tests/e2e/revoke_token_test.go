...
// Test token revocation impact on publish (integration test)
func TestRevokeTokenPublishValidation(t *testing.T) {
	assert := assert.New(t)

	pn := pubnub.NewPubNub(pamConfigCopy())
	testChannel := randomized("revoke_publish_test")

	// Create a token with write permissions
	grantRes, _, err := pn.GrantToken().TTL(5). // 5 minutes
							Channels(map[string]pubnub.ChannelPermissions{
			testChannel: {
				Read:  true,
				Write: true,
	...
	clientConfig := pamConfigCopy()
	tokenClient := pubnub.NewPubNub(clientConfig)
	tokenClient.SetToken(token)

	// Should be able to publish with the token
	_, status1, err1 := tokenClient.Publish().
		Channel(testChannel).
		Message("test message before revocation").
		Execute()
	...
	// Now revoke the token
	revokeRes, revokeStatus, revokeErr := pn.RevokeToken().
		Token(token).
		Execute()

	assert.Nil(revokeErr)
	assert.NotNil(revokeRes)
	...
	_, status2, err2 := tokenClient.Publish().
		Channel(testChannel).
		Message("test message after revocation").
		Execute()

	// Should get an authentication/authorization error
	assert.NotNil(err2)
	assert.True(status2.StatusCode >= 400)
	assert.True(strings.Contains(err2.Error(), "Forbidden") ||
		strings.Contains(err2.Error(), "Unauthorized") ||
		strings.Contains(err2.Error(), "403") ||
		strings.Contains(err2.Error(), "401"))
}
